...
int main() {
	...
	//ifstream datafile("D:\\Courses\\Crypto\\DES\\plaintext.txt");
	...
	char c = 0;
	 while (plaintext.length()% 8 != 0) {//Plain text less than 8 digits will automatically fill in 0
		plaintext += c;
	}
 ...
 string byte2bit(string byte) {//string to bit string
 ...
 string bit2byte(string bit) {//bit string to string
 ...
 string hex2bit(string hex) {//Hexadecimal string to bit string
 ...
 string bit2hex(string bit) {//bit string to hexadecimal string
 ...
 string transform(string bit, TABLE* table, int length) {//Matrix replacement
	string tmp(length, 0);
	for (int i = 0; i < length; i++) {
		tmp[i] = bit[table[i] - 1];
	}
 ...
 void get_subkey(string* subkey, string key) {//Get subkey
 ...
 string string_xor(string a, string b) {//Binary string XOR
 ...
 string function(string R, string K) {//f function
 ...
 string iterative(string L, string R, string* K, MODE mode) {//16 iterations
	if (mode == en) {
		for (int i = 0; i < 16; i++) {
			string tmp(L);
			L = R;
 ...
 string des(string data, string key, MODE mode) {//DES implements single block encryption and decryption
	string bit_data;
	if (mode == en)
...
//  string ECB(string data, string key, MODE mode) {//Codebook mode
// 	string result;
// 	if (mode == en) {
// 		for (int i = 0; i<int(data.length() >> 3); i++) {
// 			 cout << "The first "<< i + 1 <<" block: \t";
...
// }
 ...
 string CBC(string data, string key, string init_vector, MODE mode) {//Group link mode
	string result;
	string block;
