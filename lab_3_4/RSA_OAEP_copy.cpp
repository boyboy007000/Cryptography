...
void RSA_encrypt(string plaintext,string publicKey);
void create_key(string filename);
int main(int argc, char* argv[])
{
    ...
    // {
        ...
        create_key("abc");
    //     LoadPrivateKey("rsa-private-abc.key", privateKey);
    //     LoadPublicKey("rsa-public-abc.key", publicKey);
...
void create_key(string filename)
{
        ...
        if (!privateKey.Validate(rng, 3)) {
            std::cout << "Could not validate key" << endl;
        }
        SavePrivateKey("rsa-private-"+filename+".key", privateKey);
        SavePublicKey("rsa-public-"+filename+".key", publicKey);
}
